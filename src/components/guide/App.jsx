  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl h-[80vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-t-xl">
    ...
    <div className="fixed bottom-6 right-6 z-50 notification-container">
      <div className="relative">
        ...
        <button
          ...
          className="relative bg-blue-600 p-4 rounded-full shadow-lg border-2 border-white hover:shadow-xl transition-all duration-300 hover:scale-110"
        >
...
// Guide-specific notification management function
const getGuideUserNotifications = (userId, callback) => {
  try {
...
export default function GuideMain({ user, onLogin, onRegister, onLogout, onShowAuth, notifications, onNotificationClick, onMarkAsRead }) {
  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
      console.log(`🔐 GuideMain Auth state changed:`, user ? `User ${user.uid} logged in` : 'User logged out');
      setCurrentUser(user);
    });

    return () => {
      console.log('🔴 Cleaning up GuideMain auth listener');
      unsubscribeAuth();
    };
  }, []);
...
// Export the guide-specific notification function with a different name
export { getGuideUserNotifications };